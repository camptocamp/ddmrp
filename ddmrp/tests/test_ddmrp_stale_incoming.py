...
class TestDdmrpStaleIncoming(TestDdmrpCommon):
    ...
    def test_auto_procure_with_stale_incoming_qty(self):
        ...
        # A demand-only refresh leaves the supply side on its stored value, so
        # the net flow position it produces is half stale. That is precisely
        # why it may not be used to decide a procurement.
        self.assertEqual(buffer.incoming_dlt_qty, 0.0)

        # Once both sides are refreshed the buffer is seen as covered, and the
        # full refresh does not procure either.
        buffer.cron_actions()
